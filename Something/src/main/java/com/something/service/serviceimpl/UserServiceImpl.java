...
public class UserServiceImpl implements UserService, UserDetailsService {
	...
	public void updateMaxOut(User user, double amount, String type) {
		...
		if (type.equalsIgnoreCase("buy")) {
			user.setMaxout(user.getMaxout() + amount * 3);
			user.setMaxoutleft(user.getMaxoutleft() + amount * 3);
		} else if (type.equalsIgnoreCase("receive")) {
			user.setMaxout(user.getMaxout() + amount);
			user.setMaxoutleft(user.getMaxoutleft() + amount);
		} else if (type.equalsIgnoreCase("transfer") || type.equalsIgnoreCase("swap")) {
			user.setMaxoutleft(user.getMaxoutleft() - amount);
		}
	}
			...
			for (User item : userTree) {
				...
				if (item.getUsername() == null) {
					...
					if (next.size() == 0) {
						...
						if (next.get(0).getSide().equalsIgnoreCase("left")) {
							next.add(new User());
						} else if (next.get(0).getSide().equalsIgnoreCase("right")){
							temp = next.get(0);
							next = new ArrayList<>();
							next.add(new User());
							next.add(temp);
						...
						for (int t = 0; t < next.size(); t++) {
							if (next.get(0).getSide().equalsIgnoreCase("right")) {
